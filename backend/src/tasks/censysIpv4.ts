...
export const handler = async (commandOptions: CommandOptions) => {
  ...
  const { data: { results } } = await axios.get(CENSYS_IPV4_ENDPOINT, { auth });

  const { data: { files } } = await axios.get(results.latest.details_url, { auth });
  ...
  for (let fileName in files) {
    ...
    await new Promise((resolve, reject) =>
      https.get(files[fileName].download_path, {
