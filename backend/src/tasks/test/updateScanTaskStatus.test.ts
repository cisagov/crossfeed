import { handler as updateScanTaskStatus, EventBridgeEvent } from '../updateScanTaskStatus';
...
const createSampleEvent = ({lastStatus, taskArn, exitCode = 0}): EventBridgeEvent => ({
    "detail": {
        "attachments": [
        ],
...
test('starting event', async () => {
    ...
    await updateScanTaskStatus(createSampleEvent({lastStatus: "RUNNING", taskArn }), {} as any, () => null);
...
test('finished event', async () => {
    ...
    await updateScanTaskStatus(createSampleEvent({lastStatus: "STOPPED", taskArn, exitCode: 0 }), {} as any, () => null);
    scanTask = (await ScanTask.findOne(scanTask.id))!;
...
test('failed event', async () => {
    ...
    await updateScanTaskStatus(createSampleEvent({lastStatus: "STOPPED", taskArn, exitCode: 1 }), {} as any, () => null);
    scanTask = (await ScanTask.findOne(scanTask.id))!;
