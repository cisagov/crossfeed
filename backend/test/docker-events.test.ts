...
test('should listen to a start event', async () => {
    ...
    await waitForExpect(() => {
        expect(updateScanTaskStatus).toHaveBeenCalledWith({"detail": {"containers": [{}], "lastStatus": "RUNNING", "stopCode": "", "stoppedReason": "", "taskArn": "crossfeed_worker_cisa_censys_2681225"}}, expect.anything(), expect.anything());
    });
});
...
test('should listen to a stop event', async () => {
    ...
    listenForDockerEvents();
    await waitForExpect(() => {
        expect(updateScanTaskStatus).toHaveBeenCalledWith({"detail": {"containers": [{"exitCode": 0}], "lastStatus": "STOPPED", "stopCode": "EssentialContainerExited", "stoppedReason": "Essential container in task exited", "taskArn": "crossfeed_worker_cisa_censys_2681225"}}, expect.anything(), expect.anything());
    });
})